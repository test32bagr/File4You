...
class Mobile_Detect {
     ...
     * @param array $headers Specify the headers as injection. Should be PHP _SERVER flavored. If left empty, will use the global _SERVER['HTTP_*'] vars instead.
     * @param string $userAgent Inject the User-Agent header. If null, will use HTTP_USER_AGENT from the $headers array instead.
     */
     ...
     * @param array $httpHeaders The headers to set. If null, then using PHP's _SERVER to extract the headers. The default null is left for backwards compatibilty.
     */
     ...
     * @param string $header The name of the header to retrieve. Can be HTTP compliant such as "User-Agent" or "X-Device-User-Agent" or can be php-esque with the all-caps, HTTP_ prefixed, underscore seperated awesomeness.
     * @return string|null The value of the header.
     */
    public function getHttpHeader($header) {
        if (strpos($header, '_') === false) $header = strtoupper(str_replace('-', '_', $header));
        $altHeader = 'HTTP_' . $header;
        if (isset($this->httpHeaders[$header])) return $this->httpHeaders[$header];
        elseif (isset($this->httpHeaders[$altHeader])) return $this->httpHeaders[$altHeader];
        return null;
    }
    public function getMobileHeaders() { return self::$mobileHeaders; }
    /**
    ...
    public function setUserAgent($userAgent = null) {
        if (!empty($userAgent)) { return $this->userAgent = $userAgent; }
		else {
            $this->userAgent = null;
            foreach($this->getUaHttpHeaders() as $altHeader) if(!empty($this->httpHeaders[$altHeader])) $this->userAgent .= $this->httpHeaders[$altHeader]." ";
    ...
    public function setDetectionType($type = null) {
        if ($type === null) $type = self::DETECTION_TYPE_MOBILE;
        if ($type != self::DETECTION_TYPE_MOBILE && $type != self::DETECTION_TYPE_EXTENDED) return;
    ...
    public function getRules() {
        if ($this->detectionType == self::DETECTION_TYPE_EXTENDED) return self::getMobileDetectionRulesExtended();
        else return self::getMobileDetectionRules();
    }
    /**
        ...
        foreach($this->getMobileHeaders() as $mobileHeader => $matchType){
            if( isset($this->httpHeaders[$mobileHeader]) ){
                if( is_array($matchType['matches']) ){
                    foreach($matchType['matches'] as $_match) if(strpos($this->httpHeaders[$mobileHeader], $_match) !== false ) return true;
                    return false;
                } else { return true; }
            }
        }
    ...
    public function __call($name, $arguments) {
        if (substr($name, 0, 2) != 'is') { throw new BadMethodCallException("No such method exists: $name"); }
    ...
    protected function matchDetectionRulesAgainstUA($userAgent = null) {
        foreach ($this->getRules() as $_regex) {
            ...
            if ($this->match($_regex, $userAgent)) return true;
        }
        return false;
    }
    /**
    ...
    public function isMobile($userAgent = null, $httpHeaders = null) {
        ...
        $this->setDetectionType(self::DETECTION_TYPE_MOBILE);
        if ($this->checkHttpHeadersForMobile()) return true;
    ...
    public function isTablet($userAgent = null, $httpHeaders = null) {
        $this->setDetectionType(self::DETECTION_TYPE_MOBILE);
        foreach (self::$tabletDevices as $_regex) if ($this->match($_regex, $userAgent)) return true;
        return false;
    }
    /**
    ...
    public function prepareVersionNo($ver) {
        ...
        if (isset($arrVer[1])) $arrVer[1] = @str_replace('.', '', $arrVer[1]);
        return (float) implode('.', $arrVer);
    }
    /**
     * Check the version of the given property in the User-Agent.
     * Will return a float number. (eg. 2_0 will return 2.0, 4.3.1 will return 4.31)
     * @param string $propertyName The name of the property. See self::getProperties() array keys for all possible properties.
